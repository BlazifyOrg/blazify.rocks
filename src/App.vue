  ...
  <v-app>
    <v-main>
      ...
      <div class="text-center ma-2" v-if="showSnackbar">
        <v-snackbar v-model="snackbar" :light="!$vuetify.theme.dark" timeout="20000">
          Save theme, see more <router-link to="/theme-legal">here</router-link>
          <template v-slot:action="{ attrs }">
          <v-btn text v-bind="attrs" @click="snackbar = false; save()">Agree</v-btn>
            <v-btn text v-bind="attrs" @click="snackbar = false">Disagree</v-btn>
          </template>
        </v-snackbar>
      </div>
      <v-footer padless fixed>
        <v-col class="text-center" cols="12">
          ...
          <v-icon class="float-right" @click="changeTheme">mdi-theme-light-dark</v-icon>
        </v-col>
      </v-footer>
    </v-main>
  </v-app>
